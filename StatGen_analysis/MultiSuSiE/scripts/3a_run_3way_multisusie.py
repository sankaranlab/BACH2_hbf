...
def load_finemap_files(prefix, populations=["EUR", "AFR", "THAI"]):
    """
...
def parse_multisusie_result(result, snp_info, populations):
    """
...
def write_pip_effects_table(parsed, snp_info, output_prefix, populations=["EUR", "AFR", "THAI"]):
    """
...
def plot_pip_manhattan(z_list, snp_info, parsed, output_prefix, populations=["EUR", "AFR", "THAI"]):
    """
...
def plot_diagnostics(parsed, output_prefix, populations=["EUR", "AFR", "THAI"]):
    """
    Create one combined diagnostics figure.

    Panels:
      1) pairwise effect size comparisons (3 choose 2 = 3 panels) colored by PIP
    ...
    axes_scatter = [axes[0, 0], axes[0, 1], axes[1, 0]]
    ax_cs_quality = axes[1, 1]
    ax_cs_size = axes[2, 0]
    ax_conv = axes[2, 1]

    if coef is None or pip is None or coef.shape[0] < 2:
        for ax_scatter in axes_scatter:
        ...
        for ax_scatter, (i, j) in zip(axes_scatter, pair_indices):
            if coef.shape[0] <= j:
                ax_scatter.text(0.5, 0.5, "Effect comparison unavailable", ha="center", va="center", transform=ax_scatter.transAxes)
                ax_scatter.set_title("Cross-population effect comparison")
                continue
            ...
            ax_scatter.set_xlabel(f"Effect size {populations[i]}", fontsize=10)
            ax_scatter.set_ylabel(f"Effect size {populations[j]}", fontsize=10)
            ax_scatter.set_title(f"Effect comparison: {populations[i]} vs {populations[j]}")
...
def main():
    parser = argparse.ArgumentParser(
        description="Run MultiSuSiE on multi-ancestry fine-mapping data"
    )
    parser.add_argument(
