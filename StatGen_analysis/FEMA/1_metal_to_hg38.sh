...
DBSNP_VCF="${LABSHARE}/public_datasets/dbSNP/build_157/VCF/GCF_000001405.40.gz"
OUTDIR="${WDIR}/fema/hg38_output"
mkdir -p "$OUTDIR"
...
# Per-job scratch directory (environment-specific; adapt to your system).
TMPDIR="/path/to/scratch/bach2_finemap/step0d_metal_to_hg38_${SLURM_JOB_ID}"
mkdir -p "$TMPDIR"
...
bcftools view --threads "$THREADS" -i "ID=@$TMPDIR/rsids.txt" "$DBSNP_VCF" -Ou \
    ...
    ' \
    | sort --parallel="$THREADS" -t $'\t' -k1,1 > "$TMPDIR/rsid_to_hg38.sorted.tsv"

echo "[$(date)] Preparing input data"
read_input | awk 'BEGIN{FS=OFS="\t"} NR==1{print > "'$TMPDIR'/header.tsv"; next} {print}' \
    > "$TMPDIR/input_data.tsv"

echo "[$(date)] Annotating input with hg38 coordinates (awk left join)"
awk 'BEGIN{FS=OFS="\t"}
...
' "$TMPDIR/rsid_to_hg38.sorted.tsv" "$TMPDIR/input_data.tsv" > "$TMPDIR/with_coords.unsorted.tsv"
